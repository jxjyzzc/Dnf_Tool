...
from map.FbycMiniMapTools import miniMapTools
...
roomExport = loadRoomExport(r"map\xml\FengbaoYoucheng.xml")
...
total_skill,room_skill = loadJob('阿修罗','fbyc')
print("---------------------------")
...
def autoBeatMonster():
    ...
    try:
        while True:
            ...
            if playerSpeed['b'] == 0:
                print('测试人物跑动移速...')
                room_image_path = f"map/img/fbyc/fbyc_{room_id}.jpg"
            ...
            if playerSpeed['b'] == 1 and len(monsters) == 0  and len(articles) == 0:
                ...
                # siftLoc检测通道数要匹配上
                im_opencv = cv2.cvtColor(im_opencv, cv2.COLOR_BGR2RGB)
                room_image_path = f"map/img/fbyc/fbyc_{new_room_id}.jpg"
                if os.path.exists(room_image_path):
                # 人物在整个房间中的位置
                    x,y = siftLoc.getSiftLoc(room_image_path,im_opencv)

...
roomExport = loadRoomExport(r"map\xml\BaiYunDengTa.xml")
...
total_skill,room_skill = loadJob('阿修罗','bydt')
print("---------------------------")
...
def autoBeatMonster():
    ...
    while True:
        ...
        new_boss_roomid = miniMapTools.getBossRoomId()
        liefeng_roomid = miniMapTools.getLiefengRoomId()
        # 会影响识别boss逻辑，需要另调整
        # jingying_roomid = miniMapTools.getJingyingRoomId()

        if boss_roomid is None:
            boss_roomid = new_boss_roomid
        ...
        if room_id is None:
            ...
            # elif jingying_roomid is not None:
            #     print('精英房间,随意技能')
            #     # room_id = jingying_roomid
            #     room_id = '0_0'
            # 裂缝出现则不认为目前在boss房间        
        ...
        if playerSpeed['b'] == 0:
            print('测试人物跑动移速...')
            room_image_path = f"map/img/bydt/bydt_{room_id}.jpg"
        ...
        if playerSpeed['b'] == 1 and len(monsters) == 0  and len(articles) == 0:
            ...
            if new_room_id!=room_id:
                continue
            if new_room_id == boss_roomid:
                print('当前已在boss图,不进行自动过图')
                continue
            ...
            # siftLoc检测通道数要匹配上
            im_opencv = cv2.cvtColor(im_opencv, cv2.COLOR_BGR2RGB)
            room_image_path = f"map/img/bydt/bydt_{new_room_id}.jpg"
            if os.path.exists(room_image_path):
            # 人物在整个房间中的位置
                x,y = siftLoc.getSiftLoc(room_image_path,im_opencv)
            ...
            print(new_room_id,"门的坐标 x:",export_x,'y:',export_y)
